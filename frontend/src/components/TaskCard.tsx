  ...
  return (
    ...
    >
      <div className="p-4">
        ...
        {/* Result */}
        {task.result && (
          <div className="mt-3 rounded-lg bg-white/60 px-3 py-2.5 text-xs leading-relaxed text-slate-500 whitespace-pre-wrap max-h-32 overflow-y-auto">
            {task.result}
          </div>
        )}
      </div>
      ...
      {/* Action buttons */}
      <div className="mt-auto flex items-center gap-1.5 border-t border-slate-100/40 px-4 py-2 opacity-0 transition-opacity group-hover:opacity-100">
        {(task.status === 'pending' || task.status === 'failed') && (
          <button
            onClick={(e) => { e.stopPropagation(); onSkip(task._id); }}
            className="rounded-md px-2.5 py-1 text-[11px] font-medium text-amber-600 hover:bg-amber-50 transition-colors"
          >
            Ignorer
          </button>
        )}
        {(task.status === 'skipped' || task.status === 'failed') && (
          <button
            ...
            className="rounded-md px-2.5 py-1 text-[11px] font-medium text-blue-600 hover:bg-blue-50 transition-colors"
          >
            Reprendre
          </button>
        )}
        ...
        <button
          ...
          className="rounded-md px-2.5 py-1 text-[11px] font-medium text-rose-500 hover:bg-rose-50 transition-colors"
          >
          Supprimer
        </button>
      </div>
